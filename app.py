...
def create_pages():
    ...
    # プロフィールエリア (さらに下の 2600pxから開始)
    d2.rectangle([0, 2600, WIDTH, HEIGHT], fill=(245, 245, 245))
    ...
    if user_photo:
        ...
        photo.putalpha(mask)
        p2.paste(photo, (SAFE_L, 2750), photo) # 写真を下げました
    
    # テキスト (写真の横 X=950)
    d2.text((950, 2850), title, font=f(75), fill=BLACK)
    d2.text((950, 3050), name, font=f(180), fill=BLACK)
    
    # QRコード (右端 X=1950 側に寄せて、文字切れを防止)
    if qr_code:
        qr = Image.open(qr_code).resize((450, 450))
        qr_x = 1950 
        p2.paste(qr, (qr_x, 2750)) # QRコードを下げました
        # 確実に「はこちら」が入るように中央揃えの位置を調整
        d2.text((qr_x + 225, 3300), "公式LINEはこちら", font=f(70), fill=BLACK, anchor="mm")

    pdf_buf = io.BytesIO()
    p1.save(pdf_buf, format="PDF", save_all=True, append_images=[p2], resolution=300.0)
    return pdf_buf.getvalue()

if st.button("🚀 最終確定版チラシを生成する"):
    if not user_photo or not qr_code:
        st.warning("写真とQRコードが必要です")
    else:
        with st.spinner("プロ品質のチラシを生成中..."):
            pdf = create_pages()
            st.success("✅ 下部の配置を修正し、文字切れも解消しました。これで完成です！")
            st.download_button("📥 完成した両面PDFを保存", pdf, f"FP_Hosokawa_Full_Correct.pdf", "application/pdf")
