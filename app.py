...
# --- アプリ画面 ---
st.title("📄 細川様モデル：FPチラシ生成システム【レイアウト完全修正版】")
...
def create_pages():
    ...
    d1.text((WIDTH//2, 160), "節約・貯蓄・NISA・投資・保険", font=f(100), fill=WHITE, anchor="mm")
    ...
    p1.paste(graph, (WIDTH//2 - 1000, 950))
    
    # グラフ下の説明文 (Y=2200から開始)
    msg1 = f"毎月3.3万円の積立でも、30年後には {final_man:,}万円 に。\n投資元本1,188万円に対し、運用益だけで {profit_man:,}万円以上 の利益になります！"
    wrapped_msg1 = wrap_text(msg1, f(85), CONTENT_W)
    d1.multiline_text((WIDTH//2, 2350), wrapped_msg1, font=f(85), fill=BLACK, anchor="mm", align="center", spacing=30)

    # ピンクの帯 (Y=2600)
    d1.rectangle([0, 2600, WIDTH, 2950], fill=PINK)
    d1.text((WIDTH//2, 2775), f"つみたてだけで老後 {final_man}万円 を作れます！", font=f(125), fill=WHITE, anchor="mm")
    
    # 1P最下部のテキスト
    h_info = "細川の運用実績から、利回りが7.5%に収斂するという確信を得て、このサービスを開始しました。"
    d1.text((WIDTH//2, 3200), wrap_text(h_info, f(75), CONTENT_W), font=f(75), fill=BLACK, anchor="mm", align="center")
    ...
    # anchor="mm"は使わず、左上固定(SAFE_L)で描画
    ...
    # 2. テキスト (写真の右に大きく離して配置: X=1150)
    d2.text((1150, 2750), title, font=f(85), fill=BLACK)
    d2.text((1150, 2950), name, font=f(180), fill=BLACK)
    
    # 3. QRコード (右端)
    if qr_code:
        qr = Image.open(qr_code).resize((500, 500))
        # 右端からSAFE_L(250)離した位置
        qr_x = WIDTH - SAFE_L - 500
        p2.paste(qr, (qr_x, 2600))
        d2.text((qr_x + 250, 3200), "公式LINEはこちら", font=f(70), fill=BLACK, anchor="mm")

    pdf_buf = io.BytesIO()
    p1.save(pdf_buf, format="PDF", save_all=True, append_images=[p2], resolution=300.0)
    return pdf_buf.getvalue()

if st.button("🚀 最新版：A4両面チラシを生成する"):
    if not user_photo or not qr_code:
        st.warning("写真とQRコードをアップロードしてください。")
    else:
        with st.spinner("プロ品質のチラシを生成中..."):
            pdf = create_pages()
            st.success("✅ レイアウトを完全に修正したチラシが完成しました！")
            st.download_button("📥 両面PDFを保存", pdf, f"FP_Hosokawa_Perfect_Report.pdf", "application/pdf")
